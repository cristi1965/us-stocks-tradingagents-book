...
const STAGE_TALK=[
  ...
  ['我等到回踩了，限价单却没成交。','没成交只是没缘分，追成市价单可能变孽缘。','订单先写愿意牺牲价格还是成交概率。','没买到不亏钱，也算半个好消息。'],
  ['五只科技股，怎么像同一只在跌？','衣服颜色不同，雷雨来了都站同一座山头。','按风险来源合并，不按股票代码数数。','五个碗里装同一锅粥。'],
  ['CPI 出来前，我猜对数字不就行了？','还得猜市场原来押了什么。答案对了，考卷可能早被价格批完。','事件前先降到能承受跳空的仓位。','锣响之前，先把耳朵捂好。'],
  ['股价涨了，Call 怎么还亏？','你买的不只方向，还有时间和热闹。热闹散了，票也会便宜。','先写最大权利金损失，再谈 Delta。','原来烟花票过了时辰也会变纸。'],
  ['三个 Agent 都看多，可以下单了吗？','三个人抄同一条旧消息，只算一个人说话。','缺来源、时间或反证，风险门直接关闭。','嗓门不能当证据，链接可以。'],
  ['这次计划写完，我是不是会赢？','不会。它只能让你输得明白，赢得可重复。','合格流程允许亏损，不允许失控。','账记清了，下回才知道哪口锅漏。']
];
let replayState={};
...
function stageRecap(){
  ...
  const box=document.createElement('section');box.className='stage-recap';box.innerHTML=`<small>第 ${v2Stage(v2Active)+1} 幕复盘</small><h3>${V2_STAGES[v2Stage(v2Active)][0]}：流程证据已封存</h3><div><span>本幕决策 <b>${events.length}</b></span><span>账户变化 <b>${delta>=0?'+':''}$${delta.toLocaleString()}</b></span><span>当前净值 <b>$${last.toLocaleString()}</b></span></div><p>盈亏不是通关标准；是否按事前规则行动，才是。</p>`;
  lessonStage.querySelector('.lesson-nav').before(box);
}
const replayBaseComplete=v2Complete;
v2Complete=function(ok,button){
  if(V2_TITLES[v2Active][1]==='chart'&&!v2Done.has(v2Active)&&replayCount()<=5){lessonFeedback.textContent='先锁定观察，再揭晓一根新 K 线；不能偷看答案后补判断。';return}
...
v2Render=function(){
  ...
  if(V2_TITLES[v2Active][1]==='chart'){
    ...
    const reveal=document.createElement('button');reveal.id='revealBar';reveal.textContent='锁定观察，揭晓下一根 K 线';
