...
['#chapters','#chart','#risk','#options','#advancedLabs','#agents','#mastery'].forEach(s=>{const n=document.querySelector(s);if(n)n.classList.add('tool-hidden')});
function v2Stage(index){let n=0;for(let i=0;i<V2_SIZES.length;i++){n+=V2_SIZES[i];if(index<n)return i}return 7}
function v2Save(){localStorage.setItem('manga-us-v2-active',v2Active);localStorage.setItem('manga-us-v2-done',JSON.stringify([...v2Done]));localStorage.setItem('manga-us-v2-equity',v2Equity)}
