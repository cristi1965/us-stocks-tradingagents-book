...
v2Control=function(type,index){
  if(index===1)return '<div class="number-task"><p>账户 $100,000，单笔风险上限是多少？</p><label>风险比例（%）<input id="riskAnswer" type="number" step="0.1"></label><label>允许亏损（美元）<input id="lossAnswer" type="number"></label></div><button data-run>核对预算</button>';
  ...
  if(index===20)return '<div class="mini-chain"><button data-pick="good">30D · Bid 2.95 / Ask 3.10 · OI 12,400</button><button data-pick="bad">7D · Bid 1.20 / Ask 2.10 · OI 18</button></div><p>先比较点差占权利金比例、期限和未平仓量。</p>';
  if(index===21)return '<div class="number-task"><p>1 张 Call，Delta 0.50，等效多少股？</p><label>等效股数<input id="deltaAnswer" type="number"></label></div><button data-run>核对 Delta</button>';
  if(index===22)return '<div class="number-task"><p>Vega 0.12，IV 下降 10 个百分点；忽略其他因素，单份期权约变化多少美元？</p><label>价格变化<input id="vegaAnswer" type="number" step="0.1"></label></div><button data-run>核对 Vega</button>';
  if(index===24)return '<div class="number-task"><p>买 105 Call、卖 115 Call，净支出 $3。</p><label>最大亏损（每张）<input id="spreadLoss" type="number"></label><label>最大盈利（每张）<input id="spreadProfit" type="number"></label></div><button data-run>核对价差</button>';
...
v2Journal=function(){v3BaseJournal();const list=document.createElement('div');list.className='journal-events';list.innerHTML='<b>最近决策</b>'+v3Events.slice(-4).reverse().map(e=>`<span>第${e.chapter}章 · ${e.title}<small>${e.pnl>=0?'+':''}$${e.pnl}</small></span>`).join('');openTools.before(list)};
v2Render();
