...
explainInvalid = function() {
  ...
  if (!(gapPct > 0)) fieldError('planGapPct', '隔夜跳空比例必须大于 0。');
  if (!(clusterLoss >= 0)) fieldError('planClusterLoss', '同一类风险的已有持仓在坏情况下的损失不能小于 0。');
  if (!(planned > 0) || Math.abs(planned - (+planLoss.value)) > .01) fieldError('planLoss', '计划止损损失算错了：(|入场价 - 止损价| + 每股滑点与费用) × 股数。');
  if (!(gapLoss > 0) || Math.abs(gapLoss - (+planGapLoss.value)) > .01) fieldError('planGapLoss', '跳空损失算错了：(入场价 × 跳空比例 ÷ 100 + 每股滑点与费用) × 股数。');
  if (totalRisk > limit) fieldError('planClusterLoss', '这笔交易按止损和跳空两种情况算出的较大损失，加上同一类风险的已有持仓在坏情况下的损失，合计超过当前净值的 2%。');
